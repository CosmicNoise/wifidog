...
char *parse_portal_site(char *host)
{
	struct in_addr *h_addr;
	if (NULL == host){
		debug(LOG_NOTICE, "Portal Site is no set");
		return NULL;
	}
	...
	if (h_addr){
		free(host);
		return safe_strdup(inet_ntoa(*h_addr));
	}
	return NULL;
}
		...
		if (p1 && p1[0] != '\0') {
			...
			if ((strncmp(s, "#", 1)) != 0) {
				...
				case oProxyPort:
					...
					break;
				case oPortalSite:
					config.portalsite = safe_strdup(p1);	
					break;

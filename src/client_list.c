 ...
 */
t_client         *
client_list_append(const char *ip, const char *mac)
{
    ...
    curclient->ip = ip ? safe_strdup(ip) : safe_strdup("null");
    curclient->mac = mac ? safe_strdup(mac) : safe_strdup("null");
    curclient->start_time = time(NULL);
    curclient->last_updated = time(NULL);
    ...
    debug(LOG_INFO, "Added a new client to linked list: MAC: %s",
          mac);
...
client_list_find(const char *ip, const char *mac)
{
    t_client         *ptr;
	if (NULL == ip || NULL == mac) {
		debug(LOG_ERR, "ip or mac invalid");
		return NULL;
	}
    ptr = firstclient;
    while (NULL != ptr) {

				...
				if (key && value) {
					if (strcmp(command, "CLIENT") == 0) {
						...
						if (strcmp(key, "ip") == 0) {
							...
							client->start_time = atol(value);
						}
						else if (strcmp(key, "last_updated") == 0) {
							client->last_updated = atol(value);
						}
						else {
...
main_loop(void)
{
	...
	/* maybe notice failed when wifidog restart */
	fw_destroy();

	/* check network */
	char *addr = NULL;
	do {
		addr = parse_portal_site(config->portalsite);
		if (addr){
			config->portalsite = addr;
			break;
		}
		sleep(15);	
	}while(!addr);
	...
	//thread_wdctl((void *)safe_strdup(config->wdctl_sock));
	/* Start control thread */
	result = pthread_create(&tid_ping, NULL, (void *)thread_wdctl, (void *)safe_strdup(config->wdctl_sock));
	if (result != 0) {

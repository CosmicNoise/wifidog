...
char *arp_get_ip_from_arp(const char *req_mac)
{
	FILE           *proc;
	char ip[16];
	char mac[18];
...
char *bssid_toupper(unsigned char *mac)
{
...
int iw_get_mac(char *maclist, const char *mac)
{
	char *token = NULL;
	if (!maclist || !mac){
		debug(LOG_ERR, "INVALID parameter\n");
		return 1;
	}
	
	debug(LOG_INFO, "string separate\n");
	while((token = strsep(&maclist, "|")) != NULL){
		debug(LOG_INFO, "token:%s\n", token);
		if (strncmp(mac, token, 17) == 0){
			return 0;
		}
	}
	return 1;
...
int iw_get_entry(void *iw, const char *ifname, char **maclist) 
{
	struct iwinfo_ops *(*probe)(const char *);
	struct iwinfo_ops *backend = NULL;
	struct iwinfo_assoclist_entry *e;	
	char buf[IWINFO_BUFSIZE];
	int i, len;
	int val;
	char temp[IWINFO_BUFSIZE];
	int offset = 0;

	memset(temp, 0, sizeof(temp));
	probe = dlsym(iw, "iwinfo_backend");
	if (!probe){
		debug(LOG_ERR, "load iwinfo_backend error\n");
		return -1;
	}
	debug(LOG_INFO, "ifname:%s\n", ifname);
	backend = probe(ifname);
	if (!backend){
		debug(LOG_ERR, "call iwinfo_backend error\n");
		return -1;
	}
	if (backend->assoclist(ifname, buf, &len)){
		debug(LOG_ERR, "No information available\n");
		return -1;
	}
	for (i = 0; i < len; i += sizeof(struct iwinfo_assoclist_entry))	{
		e = (struct iwinfo_assoclist_entry *) &buf[i];
		debug(LOG_INFO, "iwinfo assoclist_entry mac:%s\n", format_bssid(e->mac));
		snprintf((temp + offset), (sizeof(temp) - offset), "%s|", format_bssid(e->mac));
		offset = strlen(temp);
	}
	debug(LOG_INFO, "temp:%s\n", temp);
	i = 0;
	while(temp[i]){
		temp[i] = tolower(temp[i]);
		i++;
	}
	*maclist = safe_strdup(temp);
	return  0;
}

void iw_close(void *iw)
{
	void (*finish)(void);

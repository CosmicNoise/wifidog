...
parse_commandline(int argc, char **argv)
{
    ...
    if (strcmp(*(argv + optind), "status") == 0) {
	    ...
	    config.param[0] = strdup(*(argv + optind + 1));
		...
		if ((argc - (optind + 2)) <= 0) {
		    fprintf(stderr, "wdctl: Error: You must specify a Mac "
				    "and an IP address to auth\n");
		    usage();
		    exit(1);
	    }
	    config.param[0] = strdup(*(argv + optind + 1));
	    config.param[1] = strdup(*(argv + optind + 2));
    }
	 else {
...
wdctl_reset(void)
{
	...
	strncat(request, config.param[0], (64 - strlen(request)));
	strncat(request, "\r\n\r\n", (64 - strlen(request)));
...
wdctl_auth(void)
{
	...
	strncat(request, config.param[1], (64 - strlen(request)));
	strncat(request, "\r\n\r\n", (64 - strlen(request)));
	...
	if (strcmp(buffer, "Yes") == 0) {
		printf("Connection %s (%s) successfully auth.\n", config.param[0], config.param[1]);
	} else if (strcmp(buffer, "No") == 0) {
		printf("Connection %s (%s) is unauthorized users.\n", config.param[0], config.param[1]);
	} else {
		fprintf(stderr, "wdctl: Error: WiFiDog sent an abnormal "
				"reply.\n");
	}
